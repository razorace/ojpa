...
rem Overview
rem ****************
rem Purpose:  This batch file generates a .zip build of OJP for linux using the assets in the local workspace.
rem           The filename is based on the subversion revision the local workspace is synced to.
rem
rem Requirements:
rem - mingw32 (with mingw make component) to be installed.  
rem - (mingw install folder)\bin folder in the PATH system environmental variable
